...
if n_selected~=2
  gsmc.errordlg('Can only calculate coherency between two signals at a time.',...
           'Error');
  return;
end
...
if isempty(n_sweeps) || n_sweeps~=1
  gsmc.errordlg('Must have exactly one sweep to calculate coherogram.',...
           'Error');
  return;
end
...
params=gsmc.get_coherogram_params(tl_view,dt);
...
if N_window<=1
  ...
  drawnow('update');
  drawnow('expose');
  gsmc.errordlg(['The requested window duration (and the sampling rate of ' ...
...
if di_window<1
  ...
  drawnow('update');
  drawnow('expose');
  gsmc.errordlg(['The requested window duration and number of steps per ' ...
