function retval=get_spectrogram_params(self,tl,dt)
...
param_str=self.inputdlg({ 'Window duration (s):' , ...
...
if isempty(T_window_want)
  self.errordlg('Window duration not valid','Error');
  retval=struct([]);
  return;
end
if T_window_want<=0
  self.errordlg('Window duration must be >= 0','Error');
  retval=struct([]);
  return;
end
if T_window_want>T_view
  self.errordlg(sprintf(['Window duration must be less than the view ' ...
...
if n_steps_per_window_want~=round(n_steps_per_window_want)
  self.errordlg('Number of steps per window must be an integer','Error');
  retval=struct([]);
  return;
end
if n_steps_per_window_want<1
  self.errordlg('Number of steps per window must be >= 1','Error');
  retval=struct([]);
  return;
end
