...
try
  ...
  self.errordlg(sprintf('Unable to open file %s',filename_local), ...
           'Unable to open file');  
  return;
end
...
if ~groundswell.all_time_bases_valid(t_each)
  self.errordlg('At least one signal has an invalid time base.',...
           'Invalid time base');
  self.view.unhourglass();
  return;
end
...
if groundswell.all_on_same_time_base(t_each)
  ...
  if ismac()
    button=self.questdlg(['All signals not on same time base.  ' ...
