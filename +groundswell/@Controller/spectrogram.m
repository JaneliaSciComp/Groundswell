...
if n_selected~=1
  self.errordlg('Can only calculate spectrogram on one signal at a time.',...
           'Error');
  return;
end
...
if isempty(n_sweeps) || n_sweeps~=1
  self.errordlg('Must have exactly one sweep to calculate spectrogram.',...
           'Error');
  return;
end
...
params=self.get_spectrogram_params(tl_view,dt);
...
if N_window<=1
  ...
  drawnow('update');
  drawnow('expose');
  self.errordlg(['The requested window duration (and the sampling rate of ' ...
...
if di_window<1
  ...
  drawnow('update');
  drawnow('expose');
  self.errordlg(['The requested window duration and number of steps per ' ...
