...
try
  ...
  if strcmp(err.identifier,'MATLAB:imagesci:imfinfo:whatFormat')
    self.errordlg(sprintf('Unable to determine file format of %s', ...
                     filename_local),...
             'File Error');
    return;
  elseif strcmp(err.identifier,'MATLAB:load:notBinaryFile')
    self.errordlg(sprintf('%s does not seem to be a binary file.', ...
                     filename_local),...
             'File Error');
    return;
  elseif isempty(err.identifier)
    self.errordlg(sprintf('Unable to read %s, and error lacks identifier.', ...
                     filename_local),...
             'File Error');
    return;
  elseif strcmp(err.identifier,'VideoFile:UnableToLoad')
    self.errordlg(sprintf('Error opening %s.',filename_local),...
             'File Error');
    return;    
  elseif strcmp(err.identifier,'VideoFile:UnsupportedPixelType')
    self.errordlg(sprintf('Error opening %s: Unsupported pixel type.', ...
                     filename_local),...
             'File Error');
    return;    
  else
