...
switch(tag)
  ...
  case 'colorbar_edit_bounds'
    ...
    % throw up the dialog box (routed through the controller's wrapper, so the
    % test suite's test_mode applies here too)
    bounds=self.controller.inputdlg({ 'Colorbar Maximum:' , 'Colorbar Minimum:' },...
